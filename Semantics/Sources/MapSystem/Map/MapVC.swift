...
class MapVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        progressToken = NotificationCenter.default.publisher(for: .realmsPreloaded).sink {
            if let userInfo = $0.userInfo {
                let progress = userInfo[RealmSpace.UserInfoKy.preloadingProgress] as! Float
        ...
        if vm.circleOfTrust == .private {
            NotificationCenter.default.addObserver(forName: .inviteReceived, object: nil, queue: nil) { notification in
                ...
                DispatchQueue.main.async {
                    ...
                    RealmSpace.userInitiated.async {
                        MultiRealmsAccess.makeFriends(with: inviter) { profile in
                            DispatchQueue.main.async {
