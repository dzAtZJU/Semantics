...
extension LoginVC: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        RealmSpace.app.trySignUp(cred: Credentials.apple(idToken: token)) { _ in 
            RealmSpace.userInitiated.async {
                RealmSpace.userInitiated.publicRealm { publcRealm in
                    ...
                    RealmSpace.userInitiated.privatRealm { privateRealm in
                        privateRealm.tryCreateIndividual(userName: KeychainItem.currentUserName ?? String.random(ofLength: 6))
