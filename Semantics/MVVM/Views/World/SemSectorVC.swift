...
class SemSectorVC: UIPageViewController {
    ...
    init(sector sector_: Sector) {
        ...
        if firstOceanLayer == nil {
            firstOceanLayer = OceanLayer(context: appManagedObjectContext, sector: sector, proximity: 0)
        }
...
extension SemSectorVC: UIPageViewControllerDataSource {
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        if viewController is BarrierVC {
            let oceanLayer = OceanLayerDataLayer.shared.queryByProximityEnding(.max, in: sector) ?? OceanLayer(context: appManagedObjectContext, sector: sector, proximity: 0)
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        ...
        if viewController == organVC {
            let oceanLayer = OceanLayerDataLayer.shared.queryByProximityEnding(.min, in: sector) ?? OceanLayer(context: appManagedObjectContext, sector: sector, proximity: 0)
            return UINavigationController(rootViewController: OceanLayerVC(oceanLayer: oceanLayer))
        }
